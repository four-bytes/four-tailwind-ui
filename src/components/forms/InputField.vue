<template>
  <div :class="cn(props.class)">
    ...
    <div class="relative">
      <input
        :id="inputId"
        :aria-describedby="error || success || hint ? hintId : undefined"
        ...
        :class="
          cn(
            ...
            sizeClass,
            stateClasses,
            showStateIcon && (error || success) && 'pr-10',
            disabled &&
              'disabled:border-gray-100 disabled:bg-gray-50 disabled:text-gray-400 dark:disabled:border-gray-800 dark:disabled:bg-white/[0.03] dark:disabled:text-gray-500',
          )
        "
      />
      <!-- Error Icon -->
    ...
    <p v-if="error" :id="hintId" class="mt-1.5 text-xs text-error-500">
      {{ error }}
    </p>
    <p v-else-if="success" :id="hintId" class="mt-1.5 text-xs text-success-500">
      {{ success }}
    </p>
    <p v-else-if="hint" :id="hintId" class="mt-1.5 text-xs text-gray-500">
      {{ hint }}
    </p>
  </div>
</template>
